...
1 초	128 MB	19258	5367	3425	25.586%
문제
...
int bfs()
{
	while(!Q.empty())
	{
		AB temp = Q.front(); Q.pop();
		
		for(int i=0;i<SIDE;i++)
		{
			if(_safe(temp.a+dx[i],temp.b+dy[i]) && arr[temp.a+dx[i]][temp.b+dy[i]]==0)
			{
				arr[temp.a+dx[i]][temp.b+dy[i]] = arr[temp.a][temp.b]+1;
				Q.push({temp.a+dx[i],temp.b+dy[i]});
			}
		}
	}
		int maxday=0;
